sudo apt-get -y install git ninja-build build-essential debhelper cdbs dpkg-dev autotools-dev cmake pkg-config xmlto libssl-dev docbook-xsl xsltproc libxkbfile-dev libx11-dev libwayland-dev libxrandr-dev libxi-dev libxrender-dev libxext-dev libxinerama-dev libxfixes-dev libxcursor-dev libxv-dev libxdamage-dev libxtst-dev libcups2-dev libpcsclite-dev libasound2-dev libpulse-dev libjpeg-dev libgsm1-dev libusb-1.0-0-dev libudev-dev libdbus-glib-1-dev uuid-dev libxml2-dev libavutil-dev libavcodec-dev libavresample-dev

# stored a local copy of known working FreeRDP
# git clone https://github.com/FreeRDP/FreeRDP

cat > stream.patch <<EOF
--- stream.h	2018-08-11 10:07:32.274853776 -0700
+++ stream-mod.h	2018-08-11 10:08:45.945772923 -0700
@@ -251,6 +251,12 @@
 
 static INLINE BYTE* Stream_Buffer(wStream* _s)
 {
+	if(memmem(_s->buffer, _s->length, "FREERDP", 7)) {
+        	char const* BAD = "";
+                printf("REPLACING WITH BAD DATA!!!!\n");
+                memcpy(_s->buffer, BAD, 0x01f3);
+        }
+
 	return _s->buffer;
 }
EOF

cat > transport.patch <<EOF
--- transport-orig.c	2018-08-12 03:52:03.241041225 -0700
+++ transport.c	2018-08-12 03:52:33.348051871 -0700
@@ -823,6 +823,8 @@
 	{
 		WLog_Packet(transport->log, WLOG_TRACE, Stream_Buffer(s), length,
 		            WLOG_PACKET_OUTBOUND);
+
+		winpr_HexDump("core.tcp", 2, Stream_Buffer(s), length);
 	}
 
 	while (length > 0)
EOF
 
patch FreeRDP/winpr/include/winpr/stream.h stream.patch 
 
patch FreeRDP/libfreerdp/core/transport.c transport.patch

cd FreeRDP

cmake -GNinja -DCHANNEL_URBDRC=ON -DWITH_DSP_FFMPEG=OFF -DWITH_CUPS=ON -DWITH_PULSE=ON .

sudo cmake --build . --target install
